...
namespace MessagePassingComm
{
    ...
    public class Receiver : IMessagePassingComm
    {
        ...
        public bool openFileForWrite(string name, string fileTo)
        {
            try
            {
                string writePath = Path.Combine(fileTo, name);          //path to which the files transferred
    ...
    public class Sender
    {
        ...
        /*----< processing for recei ve thread >------------------------*/
        /*
        ...
        void threadProc()
        {
            while (true)
            {
                ...
                {
                    close();                            //if current msg.to != the msg.to of the last message, the connect would close.
                    if (!connect(msg.to))
                        return;
                    lastUrl = msg.to;
                    channel.postMessage(msg);
        ...
        public bool postFile(string fileName, string fileFrom, string fileTo)
        {
            ...
            try
            {
                string path = Path.Combine(fileFrom, fileName);            //path of the files to be transferred
                fs = File.OpenRead(path);
                bytesRemaining = fs.Length;
                channel.openFileForWrite(fileName, fileTo);
                while (true)
                {
    ...
    class TestPCommService
    {
        ...
        static void Main(string[] args)
        {
            ...
            TestUtilities.checkResult(testSndrRcvr(), "Comm");
            TestUtilities.putLine();
